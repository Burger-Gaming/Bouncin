...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		fun = FlxG.random.int(0, 100); // HAHAHAHAHAHAHH UNDERTALE REFERENCE GET IT!?!?!!? ALPHYS ORDERS A PIZZA!!! SO FUNNY!!!!!!!
	...
	function startIntro()
	{
		...
		var balls:String = 'titleBG';
		
		var bg:FlxSprite = new FlxSprite();
		bg.scale.set(1.2, 1.2);
		bg.antialiasing = true;
		// no me gusta codigo de espaguetis
		if (fun <= 80){ // nick orange
			bg.color = 0xFFFF6600;
			balls = 'titleBG';
		} else if(fun > 80 && fun <= 95){ // "white paper (like a sketchbook)"
			bg.color = 0xFFFFFFFF;
			balls = 'titleBG';
		} else if(fun > 95 && fun <= 100){ // eli green
			bg.color = 0xFF028E10;
			balls = 'titleBGBunker';
		}
		bg.loadGraphic(Paths.image(balls));
		bg.screenCenter();
		add(bg);
		...
		//logoBl.y -= logoBl.width;
		logoBl.y -= logoBl.height;
		trace(logoBl.y);
		// finding location to tween lol!
		...
		var realTitleText:FlxText = new FlxText();
		realTitleText.setFormat(Paths.font("show.ttf"), 32);
		realTitleText.text = "Press Enter to Begin";
		realTitleText.screenCenter(X);
		realTitleText.y = 620;
		add(realTitleText);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Bouncin'); // credTextShit.text += '\nFunkin';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			remove(credGroup);
			skippedIntro = true;
			FlxTween.tween(logoBl, { y: 125 }, 1, { ease: FlxEase.circOut });
		}
	}
}
